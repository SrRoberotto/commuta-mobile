...
function Etapa2({ navigation }) {
  ...
  const handleCity = (item) => {
    console.log("Cidade selecionada: ",item);
    setSelectedItemCity(item.city_name);
    setCityValue(item.city_id);
    setExpandedCity(false);
  };
  ...
  async function fetchCities(state_value) {
    try {
      setLoading(true);
      console.log("Fetch cities")

      const state = state_value ? state_value : 1;

      const response = await DataServices.getCitiesByState(state)
        .then(response => {
          var count = response.data.length;
          let arrayCities = [];
          ...
          console.log ("Array Cities: ", arrayCities)
          
          setCityList(arrayCities);
          setCityDropDown(arrayCities)
          setLoading(false);
        })
        .catch(error => {
          console.log(error);
  ...
  useEffect(() => { fetchCities(); }, [cityList.length < 2]);
  useEffect(() => { fetchCities(stateValue); }, [stateValue]);
      ...
      <View>
        ...
        <View style={{ flexDirection: "row", justifyContent: 'space-between', alignItems: "center" }}>
          ...
          <View style={{ alignItems: "center", height: 55, zIndex: 10 }}>
            ...
            >
              <ScrollView style={styles.viewEstados}>
          ...
          <View style={styles.viewCidade}>
            <AccordionItem
              ...
              onPress={() => setExpandedCity(!expandedCity)}
              title={selectedItemCity || "Cidade)"}
              id="5"
              setSelectedItem={setSelectedItemCity}
              control={control} // Passa a função setSelectedItem para o AccordionItem
            >
              <ScrollView>
                {cityDropDown.length > 0 ?
                  cityDropDown.map((item) => {
                    //console.log(item);
                    return (
                      <List.Item title={item.city_name} onPress={() => handleCity(item)} />
                    );
                  }) : (
                    <List.Item title="Selecione um estado.." />
                  )}
              </ScrollView>
            </AccordionItem>
          </View>
